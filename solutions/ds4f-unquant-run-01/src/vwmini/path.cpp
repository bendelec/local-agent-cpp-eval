...
/// True when the segment [start, goal] is fully contained by the mesh and ends
/// in the cell that also contains the goal. The walk follows the segment
/// through shared edges; vertex pass-throughs resolve via the deterministic
/// epsilon probes.
[[nodiscard]] bool
segment_contained(const std::vector<Polygon> &triangles,
  ...
  for (;;) {
    ...
    // earliest one along the segment.
    Event best{};
    ...
    if (best.prio == 0) {
      ...
      if (neighbour < 0) {
        return false; // segment exits the mesh through a boundary edge
      }
