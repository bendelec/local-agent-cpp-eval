...
- **Key contracts**: `InvalidArgument`/`OutsideMesh`/`NoPath` diagnostics per SIM-001;
  ...
  deterministic portal-midpoint fallback for epsilon-band corner cases.
...
- **Key contracts**: validation of every setup/mutation call with the exact
  ...
  velocities clamped to `max_speed`; positions clamped into the mesh; arrival radius
  rule with `-1.0f` sentinel.
...
| Fixed substep `h = seconds / ceil(seconds / 0.05)` | Deterministic total integration time, bounded per-step motion, waypoint/goal clamping prevents overshoot (SIM-008). |
| Blended pairwise avoidance on a pre-update snapshot | Deterministic, local, no framework: for each close pair (i < j order) a severity is derived from the predicted closest approach (and an envelope push for pairs already too close); each moving agent blends a push directly away from its neighbour with a steer to the right of its own heading, clamps to max speed, caps motion at the next waypoint/goal, and restores containment by bisection (SIM-010–SIM-012). |
