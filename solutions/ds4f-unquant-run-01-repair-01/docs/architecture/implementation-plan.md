...
no-op, keeps terminal-state stability); same-lane pairs (dot > 0.25 within
`kSameDirectionDot`) use an anti-symmetric overtake rule — follower right,
leader left — with the side pinned per encounter (`lane_side` map) so it
never flips mid-pass; containment now wall-slides along the boundary
...
- WP8 avoidance keeps the WP6 scheme for head-on/oblique pairs (both steer
right of their own heading — already world-anti-symmetric) and adds the
anti-symmetric same-lane rule; constants `kSameDirectionDot` and
`kMaxSubsteps` documented in `simulation.cpp`.
