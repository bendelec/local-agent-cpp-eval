...
[[nodiscard]] double point_segment_distance_squared(Vec2 point, Vec2 a, Vec2 b) noexcept
{
    ...
    const double projection = std::clamp((x * edge_x + y * edge_y) / edge_length_squared,
                                         0.0, 1.0);
...
static void clip_lower(double coefficient, double constant, double& low, double& high) noexcept
{
...
static void append_circle_interval(std::vector<std::pair<double, double>>& intervals, Vec2 start,
                            Vec2 delta, Vec2 centre) noexcept
{
...
static void append_cell_intervals(std::vector<std::pair<double, double>>& intervals,
                           const NavMesh::Impl::Cell& cell, Vec2 start, Vec2 end) noexcept
{
...
[[nodiscard]] static std::vector<int> containing_components(const NavMesh::Impl& impl, Vec2 point)
{
                                              ...
                                              Vec2 start, Vec2 goal)
{
    ...
    for (std::size_t iteration = 0; iteration < count; ++iteration) {
        ...
        for (std::size_t next = 0; next < count; ++next) {
            ...
            const double candidate = distances[current]
                + std::sqrt(distance_squared(nodes[current], nodes[next]));
...
Result<NavMesh> NavMesh::create(std::vector<Polygon> triangles)
{
    ...
    for (std::size_t first = 0; first < edges.size(); ++first) {
        for (std::size_t second = first + 1; second < edges.size(); ++second) {
            if (edges[first].cell == edges[second].cell || !same_edge(edges[first], edges[second])) {
                continue;
            }
    ...
    for (std::size_t start = 0; start < impl->cells.size(); ++start) {
        ...
        while (!pending.empty()) {
            ...
            for (const int neighbour : impl->cells[cell].neighbours) {
                if (neighbour >= 0 && impl->cells[static_cast<std::size_t>(neighbour)].component == -1) {
...
Result<Path> find_path(const NavMesh& mesh, Vec2 start, Vec2 goal)
{
    ...
    const std::vector<int> start_components = MeshAccess::containing_components(*mesh.m_impl, start);
    ...
    for (const int component : start_components) {
        ...
        for (std::size_t index = 1; index < candidate->points.size(); ++index) {
            candidate_length += std::sqrt(distance_squared(candidate->points[index - 1],
