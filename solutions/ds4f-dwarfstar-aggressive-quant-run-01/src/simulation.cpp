                    ...
                    const std::vector<Snapshot>& snap, float dt)
{
    ...
    // Prefer a candidate that keeps the full safety margin.
    if (auto c = findCandidate(kAvoidMargin)) {
        return *c;
    }

    // No collision-free candidate at the strict margin: stop rather than push
    // into a neighbour (SIM-012 robustness; guarantees no overlap, never
    // teleports).
    return {0.0f, 0.0f};
}

// Applies the chosen velocity, clamping speed and never overshooting a waypoint
// or goal. Positions are kept inside the mesh.
void applyMove(SimData::Agent& a, Vec2 chosen, float dt, const NavMesh& mesh)
{
    ...
    Vec2 newpos;
    Vec2 newvel;
    if (remLen > 0.0f && dispLen >= remLen) {
        // Reach this waypoint exactly; do not overshoot.
        newpos = target;
        newvel = rem * (1.0f / dt);
