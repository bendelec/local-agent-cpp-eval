...
ctest --test-dir build --output-on-failure           # 100% tests passed out of 78  (first/second pass; the third pass is above)
...
Test suites at this stage, by registered suite name: `Vec2Test` (7), `NumericsTest` (9),
`TriangulateTest` (13), `NavMeshTest` (18), `PathTest` (10), `SimulationTest` (21) = 78, all
deterministic (fixed inputs, fixed iteration order, no time/randomness/containers keyed by floats).
The third pass adds three cases; the current total and suite counts are recorded there.
...
* Added: `src/{predicates.hpp, geometry.cpp, triangulation.{hpp,cpp}, mesh_topology.{hpp,cpp},
  ...
  steering.{hpp,cpp}, simulation.cpp, error.hpp, vec2d.hpp}`, `tests/{mesh_fixtures.hpp,
  geometry_test.cpp, triangulation_test.cpp, nav_mesh_test.cpp, numeric_test.cpp, path_test.cpp,
  simulation_test.cpp}`,
...
Two of the fixes above were themselves wrong and were removed in the third pass (see
"Repair round: motion contract defects"): the midpoint sweep it added for SIM-012 accepted
jumps across real gaps, and the residual integrator it introduced broke the speed budget. The
verification table below is the second pass's own record and was not corrected in place.
...
## Repair round: motion contract defects (third pass)

The second pass added two motion-policy mechanisms that no requirement asked for, and each one
broke a contract:

| Defect | Mechanism | Consequence | Resolution |
|---|---|---|---|
| A motion segment was accepted from an endpoint + midpoint sample | `sweep_is_inside` (`simulation.cpp`) | With a tall rectangle at `x=[-1,1]`, an island triangle at `(50,0)` and another at `(100,0)`, an agent shoved east at `100000 m/s` from `(0,0)` was stored at `(100,0)`: start, middle and endpoint each lay in walkable triangles while `(25,0)` and `(75,0)` were in the air. Not sufficient for SIM-012 | Deleted. Motion now asks `MeshTopology::segment_is_contained` — the exact interval-coverage query that certifies a planned route — and only a completely covered stored-position segment is accepted |
| `AgentRuntime::residual` banked sub-ULP motion between substeps | `agent.hpp`, `simulation.cpp` | Near `x=1e4` a `1 m/s` agent stepping `0.0001 s` jumped a whole float ULP (`0.0009765625`) on the fifth call, although only `0.0005 s` of travel had been paid for: the speed budget was exceeded | Deleted. A displacement the stored `float` position cannot represent now simply does not happen; nothing accumulates, so per-call and cumulative movement are both bounded by `max_speed * elapsed` |
| `Vec2d::to_vec2()` narrowed unconditionally, and motion called it before testing finiteness | `vec2d.hpp`, `simulation.cpp` | A double candidate outside `[-FLT_MAX, FLT_MAX]` (large finite stride plus a deflection away from the goal) was converted first and inspected afterwards, relying on implementation-defined behaviour | Replaced by `Vec2d::narrow()`, which checks finiteness and range before converting and returns `std::optional<Vec2>`. There is no unchecked narrowing call site left |

### How motion works now

* Stride capped by the distance to the target (SIM-008), computed in double.
* One exact coverage query decides an unobstructed substep. If the stride does not fit, it is
  bisected `kMotionBisectionSteps` (16) times — bounded work, remainder below one float ULP of the
  landing coordinate — and every candidate is verified before it is taken. Coverage along a ray is
  not monotone, so bisection returns a *verified* fraction rather than necessarily the largest;
  confirming the candidate, not maximising it, is what SIM-012 requires.
* `Simulation` reaches the topology through one private seam: `NavMesh` befriends `Simulation`
  (`include/vwmini/nav_mesh.hpp`) and `simulation.cpp` includes `nav_mesh_impl.hpp`. No public
  declaration, signature, default, enum value, include path or the library target changed.
* Deliberately not modelled: an agent is a point. Its radius takes part in steering, never in
  containment, so no swept disc is tested against walls.

### Tests

| Test | What it pins down |
|---|---|
| `SimulationTest.MotionNeverTeleportsThroughAGap` | the scenario above: after `step(0.001)` the agent is held at the rectangle wall (`x = 0.99992` measured), never in either island triangle, and it still routes up the rectangle and arrives |
| `NumericsTest.SmallStepsFarFromTheOriginRespectTheSpeedBudget` | per-call and cumulative `distance <= max_speed * elapsed` in double arithmetic, five calls at `x = 1e4` with `1e-4 s` steps |
| `NumericsTest.ExtremeDeflectionPastTheFloatLimitStaysFiniteContainedAndCapped` | mesh edge at `FLT_MAX`, agent at `3.39e38`, `1.5e38 m/s` deflected east by an overlapping peer: state stays finite and contained, speed cap holds, blocked peer does not drift, simulation stays usable |
| `NumericsTest.AgentSteersAcrossAnOffsetWiderThanTheFloatRange` | kept, retuned: the speed now makes one *substep's* progress representable (`1e34 m/s`) instead of relying on banked sub-ULP travel |

`tests/numeric_test.cpp` therefore holds 11 cases and `SimulationTest` 22; total 81
(`Vec2Test` 7, `NumericsTest` 11, `TriangulateTest` 13, `NavMeshTest` 18, `PathTest` 10,
`SimulationTest` 22). Nothing was deleted, no input or trajectory is hard-coded to its answer, and
pathfinding and local avoidance are untouched.

### Verification after the third pass (actual outcomes)

| Check | Result |
|---|---|
| `scripts/check.sh` — GCC 16.2.1 Debug configure, build, `ctest`, `clang-format --dry-run -Werror` | `ALL CHECKS PASSED`, 81/81, format clean |
| Release (`NDEBUG`), GCC 16.2.1 | 0 warnings, 81/81 pass |
| Clang 22.1.8, `-Wall -Wextra -Wpedantic` + strict set | 0 warnings, 81/81 pass |
| ASan + UBSan | 0 warnings, 81/81 pass, no sanitizer reports |
| `-DBUILD_TESTING=OFF` library-only build | `libvwmini.a`, no test targets, 0 warnings |
| `-DVWMINI_BUILD_TESTS=OFF` alias | configures, forwards to `BUILD_TESTING`, no test targets |
| Downstream consumer (`tests/consumer`, `BUILD_TESTING=OFF`) | builds warning-free, exits 0 |

Manual probe of the gap scenario (`step(0.001)` then 200 ordinary steps): position after the
deflected substep `(0.999921, 0)`; after the ordinary steps `(0, 150)` with status `Reached`.
