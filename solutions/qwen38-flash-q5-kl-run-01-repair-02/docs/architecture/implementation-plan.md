...
## WP8 — Numeric-scale safety (MSH-001, MSH-006, SIM-002/003/012)

| Slice | Goal | Depends on | Files / interfaces | Verification | Status |
|---|---|---|---|---|---|
| 8.1 | State one numeric policy in code and add the double working vector | 1.1 | `src/predicates.hpp`, `src/vec2d.hpp` | no other module does raw geometric arithmetic (`grep` for `cross(`/`dot(` outside the policy module) | done |
| 8.2 | Double-returning area/squared-length helpers; no new tolerance | 8.1 | `predicates.hpp`, `triangulation.cpp` | existing triangulation tests unchanged; `kMinNonDegenerateDoubleArea` still `epsilon * epsilon` | done |
| 8.3 | `length` finite for unrepresentable norms; `normalized` keeps direction | 1.1 | `geometry.cpp` | `NumericsTest.LengthStaysFiniteForExtremeFiniteVectors`, `NormalizedKeepsDirectionForExtremeAndTinyVectors` (pass in Debug **and** Release) | done |
| 8.4 | Containment, segment coverage, topology predicates decide in double | 8.1 | `mesh_topology.{hpp,cpp}`, `triangulation.cpp` | `NumericsTest.ExtremeTriangleContainsItsInteriorPoint`; all pre-existing containment tests unchanged | done |
| 8.5 | Routing, costs and centroids in double | 8.4 | `corridor.cpp`, `pathfinding.cpp` | `NumericsTest.ExtremeTriangleSupportsPathQueries`, `RoutesOnAMeshFarFromTheOrigin` | done |
| 8.6 | Motion and avoidance decide in double; a substep that is not representable does not move | 8.1 | `steering.cpp`, `simulation.cpp`, `agent.hpp` | `NumericsTest.AgentsMoveOnAMeshFarFromTheOrigin`; ASan/UBSan suite clean | done |
| 8.7 | Standard `BUILD_TESTING` for the suite | 0.1 | `CMakeLists.txt` | `-DBUILD_TESTING=OFF` configures a library-only build; default config still runs 78 tests | done |
| 8.8 | Release-active regressions + docs describing the policy | 8.3-8.7 | `tests/numeric_test.cpp`, `tests/consumer/main.cpp`, README, `architecture.md` §6 | `ctest` 78/78 in Debug/Release/Clang/ASan+UBSan; consumer exit 0 | done |
| 8.9 | Review round: no `float` operand left inside a decision, relative steering tolerances (its `residual` integrator and midpoint sweep were removed again by 8.10) | 8.3-8.8 | `src/{predicates.hpp,mesh_topology.cpp,pathfinding.cpp,steering.cpp,simulation.cpp,triangulation.cpp,agent.hpp}`, `tests/numeric_test.cpp`, README, `architecture.md` §6 | `scripts/check.sh` ALL CHECKS PASSED; 78/78 in Debug/Release/Clang/ASan+UBSan; `-DBUILD_TESTING=OFF` library only; consumer exit 0 | done |
| 8.10 | Repair round: motion contract defects — exact segment coverage for every accepted substep, no cross-substep motion banking, range-checked narrowing | 8.6, 8.8 | `src/simulation.cpp`, `src/vec2d.hpp`, `src/agent.hpp`, `include/vwmini/nav_mesh.hpp` (private seam), `tests/simulation_test.cpp`, `tests/numeric_test.cpp` | `SimulationTest.MotionNeverTeleportsThroughAGap`, `NumericsTest.SmallStepsFarFromTheOriginRespectTheSpeedBudget`, `NumericsTest.ExtremeDeflectionPastTheFloatLimitStaysFiniteContainedAndCapped` | done |

## Revisions log

* (8.10) Motion policy narrowed back to the contract. Slice 8.9's two extra mechanisms were
  removed: the endpoint+midpoint sweep (it accepted a jump across two gaps in one substep) and the
  `residual` integrator (it let observable movement pass `max_speed * elapsed`). Motion now uses
  `MeshTopology::segment_is_contained` — the query routing already relied on — reached through one
  private `NavMesh` seam, and `Vec2d::narrow()` checks its range before converting. No public
  declaration changed and no new subsystem was introduced.
...
* (WP8) numeric policy: internal geometry switched from `float` to double intermediates. Evidence:
  for the accepted triangle `(0,0) (3e38,0) (0,3e38)` the deciding cross product is `inf` in float
  and `3e76` in double, and `2.5e38 + 1.5e38` is `inf` in float — a float decision there is taken on
  an overflowed value. Public storage (`Vec2`) and the public API types are unchanged.
* (WP8) the plan's slice 8.5 originally listed a `funnel`-era "visibility recheck"; the current
  verified greedy shortening needs no extra pass — the double-domain `segment_is_contained` already
  decides every shortcut.
* (WP8) `CMakeLists.txt` now calls `include(CTest)`; the suite is gated by the standard
  `BUILD_TESTING`, with `VWMINI_BUILD_TESTS` retained as an alias that forwards to it (setting it
  OFF verified to produce a library-only build).
