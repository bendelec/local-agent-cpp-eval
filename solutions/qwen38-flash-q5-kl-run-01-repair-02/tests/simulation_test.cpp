...
/**
 * SIM-012: a substep may not teleport across ground the mesh does not cover.
 *
 * The moving agent is shoved east at full speed by an overlapping peer, so its start (0,0), the
 * midpoint (50,0) and the endpoint (100,0) all sit in walkable triangles while (25,0) and (75,0)
 * hang in the air between them. Sampling a start, an endpoint and a midpoint therefore accepted
 * the jump across both gaps; the mesh's own exact segment coverage — the query that certifies a
 * planned route — rejects it, and the agent keeps to the tall rectangle it started in.
 */
TEST(SimulationTest, MotionNeverTeleportsThroughAGap)
{
    const NavMesh mesh = fixtures::make_mesh(std::vector<fixtures::Polygon>{
        fixtures::triangle(Vec2{-1.0f, -1.0f}, Vec2{1.0f, -1.0f}, Vec2{1.0f, 151.0f}),
        fixtures::triangle(Vec2{-1.0f, -1.0f}, Vec2{1.0f, 151.0f}, Vec2{-1.0f, 151.0f}),
        fixtures::triangle(Vec2{49.0f, -1.0f}, Vec2{51.0f, -1.0f}, Vec2{50.0f, 1.0f}),
        fixtures::triangle(Vec2{99.0f, -1.0f}, Vec2{101.0f, -1.0f}, Vec2{100.0f, 1.0f})});

    Simulation simulation{NavMesh{mesh}};
    // The blocker has no goal: it only overlaps the mover and pushes it east.
    ASSERT_TRUE(simulation.add_agent(AgentConfig{Vec2{-0.5f, 0.0f}, 0.25f, 1.0f}).has_value());
    const auto id =
        simulation.add_agent(AgentConfig{Vec2{0.0f, 0.0f}, 0.25f, 100000.0f, Vec2{0.0f, 150.0f}})
            .value();

    ASSERT_TRUE(simulation.step(0.001f).has_value());
    const auto state = simulation.agent(id);
    ASSERT_TRUE(state.has_value());
    EXPECT_TRUE(mesh.contains(state->position));
    EXPECT_LT(state->position.x, 49.0f) << "the agent jumped into the first island triangle";
    EXPECT_GT(state->position.x, -1.0f);
    EXPECT_GT(state->position.y, -1.0f);
    EXPECT_LT(state->position.y, 151.0f);

    // Ordinary routing is untouched: once the peer is no longer overlapping, the agent walks up
    // the rectangle and arrives at its goal.
    for (int index = 0; index < 200; ++index)
    {
    ...
    EXPECT_LT(simulation.agent(id)->position.y, 151.0f);
}
