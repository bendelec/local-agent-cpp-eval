...
* A step shorter than one float ULP of an agent's coordinate cannot change the stored position, so
  it does not happen: far from the origin an agent may keep its coordinate. Nothing is banked
  between substeps, so movement never exceeds `max_speed * elapsed` in any single call or over any
  cumulative interval.
...
* Narrowing a double position to `Vec2` checks finiteness and the `[-FLT_MAX, FLT_MAX]` range
  *before* converting (`Vec2d::narrow()` returns `std::optional<Vec2>`): an unrepresentable
  candidate is a plain "no", never an implementation-defined `float`.

A motion substep whose landing position a `float` cannot hold is shortened, and if it still does
not fit, skipped: positions stay finite and no geometry is declared invalid. Before the agent
moves, the *whole* swept segment is checked with the same exact query that certifies a planned
route (`MeshTopology::segment_is_contained`), so an accepted substep never crosses ground the mesh
does not cover. What is not modelled: an agent is a point, so its radius takes part in steering but
never in containment.
