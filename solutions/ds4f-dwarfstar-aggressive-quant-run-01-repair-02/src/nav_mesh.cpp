...
bool trianglesOverlap(const Tri& t1, const Tri& t2)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            // Classify the two third vertices by the exact sign of the signed
            // area (no tolerance-zero): a third vertex may legitimately lie
            // within kEpsilon of the shared-edge line (thin triangles), and
            // using the tolerance here wrongly rejected valid opposite-side
            // pairs. Interiors overlap only when both third vertices sit on
            // the same side of the shared-edge line.
            const int s1 = internal::triArea2(a1, b1, c1) > 0.0f ? 1 : -1;
            const int s2 = internal::triArea2(a1, b1, c2) > 0.0f ? 1 : -1;
            if (s1 == s2) {
                return true;
            }
        }
...
// Distance from point p to the closed triangle t: 0 when strictly inside,
// otherwise the minimum distance to one of the three boundary edges. For a
// convex triangle this equals the distance from p to the triangle region.
float distToTri(Vec2 p, const Tri& t)
{
    if (internal::pointInTri(p, t.v[0], t.v[1], t.v[2])) {
        return 0.0f;
    }
    float d = internal::distPointSegment(p, t.v[0], t.v[1]);
    d = std::min(d, internal::distPointSegment(p, t.v[1], t.v[2]));
    d = std::min(d, internal::distPointSegment(p, t.v[2], t.v[0]));
    return d;
}

// Interval of the segment parameter u in [0,1] for which point(s + (g-s)*u)
// lies within kEpsilon of triangle t (inside it or near its boundary). That
// covered region is the convex triangle inflated by kEpsilon, so its
// intersection with the segment is a single interval. The distance to a convex
// set is convex in u, so a ternary search for the minimizer followed by two
// monotone bisections finds the interval exactly (to float precision).
std::optional<std::pair<float, float>> coveredInterval(Vec2 s, Vec2 g, const Tri& t)
{
    const Vec2 d = g - s;
    auto f = [&](float u) { return distToTri(s + d * u, t); };

    // Locate the minimizer of the convex distance along [0,1] via ternary search.
    float lo = 0.0f;
    float hi = 1.0f;
    for (int i = 0; i < 60; ++i) {
        const float m1 = lo + (hi - lo) * (1.0f / 3.0f);
        const float m2 = lo + (hi - lo) * (2.0f / 3.0f);
        if (f(m1) < f(m2)) {
            hi = m2;
        } else {
            lo = m1;
    ...
    if (f(m) > internal::kEpsilon) {
        return std::nullopt; // no point of the segment is covered by this triangle
    }

    float tLow;
    if (f(0.0f) <= internal::kEpsilon) {
        tLow = 0.0f;
    } else {
        // f is non-increasing on [0,m]: the predicate f<=kEpsilon flips once.
        float bl = 0.0f;
        float br = m;
        for (int i = 0; i < 60; ++i) {
            const float bm = (bl + br) * 0.5f;
            if (f(bm) <= internal::kEpsilon) {
                br = bm;
            } else {
                bl = bm;
            }
    ...
    float tHigh;
    if (f(1.0f) <= internal::kEpsilon) {
        tHigh = 1.0f;
    } else {
        // f is non-decreasing on [m,1]: the predicate f<=kEpsilon flips once.
        float bl = m;
        float br = 1.0f;
        for (int i = 0; i < 60; ++i) {
            const float bm = (bl + br) * 0.5f;
    ...
    return std::pair{tLow, tHigh};
}

// Exact continuous containment of segment [s,g] (MSH-006 / SIM-002): every
// point of the segment is inside a triangle or within kEpsilon of a triangle
// edge. Per triangle the covered parameter interval is convex; merging the
// intervals and checking coverage of [0,1] decides containment exactly, so
// narrow non-walkable gaps are never skipped by finite sampling.
bool continuousContained(const MeshData& impl, Vec2 s, Vec2 g)
{
    std::vector<std::pair<float, float>> intervals;
    for (const Tri& t : impl.tris) {
        if (auto iv = coveredInterval(s, g, t)) {
    ...
    // Float-rounding tolerance for interval joins and the [0,1] coverage test
    // (genuine uncovered gaps far wider than this are still caught).
    constexpr float kGapTol = 1e-6f;
    if (intervals[0].first > kGapTol) {
        return false; // the start point is not covered
    }
    float reach = intervals[0].second;
    for (std::size_t k = 1; k < intervals.size(); ++k) {
        if (intervals[k].first <= reach + kGapTol) {
            reach = std::max(reach, intervals[k].second);
        } else {
            return false; // a genuine uncovered gap on the segment
        }
    }
    return reach >= 1.0f - kGapTol;
}

// Direct segment is contained iff every point is inside a triangle or within
// kEpsilon of a triangle edge (MSH-006 / SIM-002).
bool directContained(const MeshData& impl, Vec2 s, Vec2 g)
{
    return continuousContained(impl, s, g);
}
                                              ...
                                              const MeshData& impl)
{
    ...
    for (std::size_t i = 0; i < N; ++i) {
        for (std::size_t j = i + 1; j < N; ++j) {
            bool visible = continuousContained(impl, nodes[i], nodes[j]);
            if (visible) {
    ...
    // chain ends with the goal node (index 1); drop it so the goal is not
    // emitted twice (once via the chain, once via the explicit push below).
    chain.pop_back();
...
Result<Path> find_path(const NavMesh& mesh, Vec2 start, Vec2 goal)
{
    ...
    // Containment is checked before the equal-endpoint special case: identical
    // endpoints that lie outside the mesh must be rejected (OutsideMesh), not
    // returned as a single-point path.
