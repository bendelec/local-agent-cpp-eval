...
static void test_length_large_finite()
{
    // Issue 6: length must stay finite for finite large/small coordinates
    // (scaled-norm implementation) and keep exactness on ordinary values.
    CHECK(std::isfinite(length(Vec2{1e20f, 1e20f})));
    CHECK(std::isfinite(length(Vec2{1e30f, 1e30f})));
    CHECK(std::isfinite(length(Vec2{1e-20f, 1e-20f})));
    CHECK(length(Vec2{1e20f, 1e20f}) > 0.0f);
    CHECK(length(Vec2{1e-20f, 1e-20f}) > 0.0f);
    CHECK(length(Vec2{3, 4}) == 5.0f);
}
