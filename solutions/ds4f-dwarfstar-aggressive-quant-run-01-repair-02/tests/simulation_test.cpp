...
// MSH-006 containment for the open 10x10 square mesh: inside the square or
// within kEpsilon of a boundary edge.
bool openMeshContained(Vec2 p)
{
    const float e = 1e-4f;
    return p.x >= -e && p.x <= 10.0f + e && p.y >= -e && p.y <= 10.0f + e;
}
...
static void test_avoidance_crossing_large_radius()
{
    // Issue 3: a crossing with larger discs (radius 0.5) used to deadlock with
    // both agents stuck Moving at the strict margin. With progressive margin
    // relaxation they must slip past and both reach their goals, while never
    // overlapping by more than 1e-3, leaving the mesh, or exceeding max_speed
    // per substep (issue 4: swept-path membership and bounded motion).
    auto r = NavMesh::create(openMesh());
    CHECK(r.has_value());
    if (!r) {
        return;
    ...
    Simulation sim(std::move(*r));
    auto a = sim.add_agent(AgentConfig{{2.0f, 5.0f}, 0.5f, 1.4f, Vec2{9.0f, 5.0f}});
    auto b = sim.add_agent(AgentConfig{{5.0f, 2.0f}, 0.5f, 1.4f, Vec2{5.0f, 9.0f}});
    CHECK(a.has_value() && b.has_value());

    float maxOverlap = 0.0f;
    bool inMesh = true;
    bool speedOk = true;
    Vec2 pa = sim.agent(*a)->position;
    Vec2 pb = sim.agent(*b)->position;
    for (int i = 0; i < 80; ++i) {
        CHECK(sim.step(0.1f).has_value());
        auto sa = sim.agent(*a);
        auto sb = sim.agent(*b);
        ...
        inMesh = inMesh && openMeshContained(sa->position) && openMeshContained(sb->position);
        speedOk = speedOk && length(sa->position - pa) <= 1.4f * 0.1f + 1e-3f &&
                  length(sb->position - pb) <= 1.4f * 0.1f + 1e-3f;
        pa = sa->position;
        pb = sb->position;
    }
    CHECK(maxOverlap <= 1e-3f);
    ...
    // Both agents must reach their goals (issue 3 deadlock regression).
    auto ea = sim.agent(*a);
    auto eb = sim.agent(*b);
    CHECK(ea.has_value() && eb.has_value());
...
static void test_avoidance_crossing_swept()
{
    // Issue 3: in an open-space crossing, each committed substep swept segment
    // must keep the two discs apart (no swept collision). This guards the
    // committed-vs-snapshot discrepancy: a candidate that clears the other's
    // snapshot-horizon segment can still overlap the other's actually committed
    // substep motion when both swerve. The committed-segment verification must
    // keep prev->pos swept segments separated whenever collision-free motion
    // exists.
    auto r = NavMesh::create(openMesh());
    CHECK(r.has_value());
    if (!r) {
    ...
    for (int i = 0; i < 60; ++i) {
        ...
        auto sb = sim.agent(*b);
        CHECK(sa.has_value() && sb.has_value());
        const float dseg = internal::distSegSeg(pa, sa->position, pb, sb->position);
        worst = std::max(worst, need - dseg);
        pa = sa->position;
        pb = sb->position;
    }
    CHECK(worst <= 1e-3f);
...
int main()
{
    ...
    test_avoidance_crossing_large_radius();
    test_avoidance_crossing_swept();
