...
static void test_find_path_direct()
{
    ...
    if (r) {
        ...
        // Issue 4: identical finite endpoints OUTSIDE the mesh -> OutsideMesh
        // (the contains() checks must precede the equal-endpoint special case).
        CHECK(find_path(m, {2.0f, 2.0f}, {2.0f, 2.0f}).error().code == ErrorCode::OutsideMesh);
        CHECK(find_path(m, {2.0f, 2.0f}, {-2.0f, -2.0f}).error().code == ErrorCode::OutsideMesh);
...
static void test_find_path_segments_contained()
{
    ...
    if (r) {
        ...
        if (p) {
            ...
            // Issue 2: the direct diagonal cuts through the non-walkable notch,
            // so continuous containment must reject it and find_path must
            // return a corridor (>= 3 points), not the raw two endpoints.
            CHECK(pts.size() >= 3);
...
static void test_find_path_narrow_gap()
{
    // Issue 2: a segment crossing a narrow non-walkable gap (wider than
    // epsilon) must not be treated as contained. The U-shaped walkable region
    // has two legs separated by a 0.02-wide mouth; the direct segment between
    // the legs crosses that mouth, so continuous containment must reject it and
    // find_path must route around the bottom connection (>= 3 points).
    Polygon U{{Vec2{0, 0}, Vec2{1.52f, 0}, Vec2{1.52f, 1.5f}, Vec2{0.52f, 1.5f}, Vec2{0.52f, 0.5f}, Vec2{0.5f, 0.5f},
               Vec2{0.5f, 1.5f}, Vec2{0, 1.5f}}};
    auto tris = triangulate_simple_polygon(U);
    CHECK(tris.has_value());
    if (!tris) {
        return;
    }
    auto r = NavMesh::create(*tris);
    CHECK(r.has_value());
    if (!r) {
        return;
    }
    const NavMesh& m = *r;
    // Crossing the mouth -> corridor route, not a direct 2-point path.
    auto p = find_path(m, {0.4f, 1.0f}, {0.6f, 1.0f});
    CHECK(p.has_value());
    if (p) {
        CHECK(p->points.size() >= 3);
        CHECK(p->points.front() == Vec2{0.4f, 1.0f});
        CHECK(p->points.back() == Vec2{0.6f, 1.0f});
    }
    // Segment staying within one leg -> still direct.
    auto q = find_path(m, {0.2f, 0.2f}, {0.4f, 0.2f});
    CHECK(q.has_value());
    if (q) {
        CHECK(q->points.size() == 2);
    }
}

static void test_create_thin_opposite_triangles()
{
    // Issue 5: two thin triangles sharing a complete edge, with their
    // non-shared third vertices on opposite sides of the shared-edge line
    // (close to it but beyond epsilon), are valid adjacent triangles. The side
    // classification must use the exact signed-area sign, not a tolerance that
    // wrongly flattens near-line vertices onto the edge.
    auto r = NavMesh::create({tri({0, 0}, {10, 0}, {5, 5e-3f}), tri({0, 0}, {5, -5e-3f}, {10, 0})});
    CHECK(r.has_value());
    if (r) {
        CHECK(r->cell_count() == 2);
    }
...
static void test_find_path_l_bend()
{
    ...
    if (r) {
        ...
        if (p) {
            ...
            // Issue 1: the goal must appear exactly once, as the final point,
            // and never as a consecutive duplicate of the previous point.
            if (pts.size() >= 2) {
                CHECK(pts[pts.size() - 2] != Vec2{0.5f, 2.5f});
            }
...
int main()
{
    ...
    test_find_path_narrow_gap();
    test_create_identical_triangles();
    test_create_thin_opposite_triangles();
    if (failures == 0) {
