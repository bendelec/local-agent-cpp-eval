...
{
    // Both operands widen to double individually: the difference of two `Vec2` would be float.
    return length_squared(Vec2d(a) - Vec2d(b)) <= kEpsilonSquared;
}
...
[[nodiscard]] bool point_inside_edge_span(Vec2 point, const Triangle &cell, std::size_t corner)
{
    ...
    return distance_squared_to_segment(point, start, end) <= kEpsilonSquared;
}
...
[[nodiscard]] bool cell_contains(const Triangle &cell, Vec2d point) noexcept
{
...
/// True when any cell contains `point` (MSH-006).
[[nodiscard]] bool cells_contain(const std::vector<Triangle> &cells, Vec2d point) noexcept
{
...
bool MeshTopology::segment_is_contained(Vec2 from, Vec2 to) const
{
    ...
    for (const Triangle &cell : m_cells)
    {
        for (std::size_t corner = 0; corner < 3; ++corner)
        {
            append_crossing_parameters(from, to, cell.corner(corner), cell.corner(corner + 1),
                                       cuts);
        }
    }
    std::sort(cuts.begin(), cuts.end());

    const Vec2d delta = Vec2d(to) - Vec2d(from);
    double previous = 0.0;
    for (const double cut : cuts)
    {
        ...
        const Vec2d sample = from + delta * (0.5 * (previous + cut));
        if (!cells_contain(m_cells, sample))
        {
            return false;
        }
        previous = cut;
