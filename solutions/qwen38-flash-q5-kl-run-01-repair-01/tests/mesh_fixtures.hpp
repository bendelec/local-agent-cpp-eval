...
/// Translate every vertex of every cell by `delta`.
[[nodiscard]] inline std::vector<Polygon> offset(const std::vector<Polygon> &cells, Vec2 delta)
{
    std::vector<Polygon> moved = cells;
    for (Polygon &cell : moved)
    {
