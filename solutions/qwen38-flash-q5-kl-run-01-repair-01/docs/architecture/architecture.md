...
4. `advance_agent` integrates it: stride capped by the distance to the target (the remainder left
   over from earlier substeps counts towards that cap), then shrunk until both ends and the midpoint
   of the swept segment are contained, then waypoint capture and `refresh_arrival`.
...
* **Stored coordinates are never rewritten.** `Vec2` remains the storage and public type;
  ...
  narrowing point (`Vec2d::to_vec2()`, and the `float` returns in `geometry.cpp`).
...
* **Motion stays representable and additive.** A substep's displacement is computed in double. If
  the landing point would not be a finite `Vec2` the agent does not move (the shortening rule of
  SIM-012), which keeps positions finite without declaring the geometry invalid. If the landing is
  representable but the step is shorter than one float ULP of the current coordinate - below the
  resolution of the stored position - the remainder is kept in `AgentRuntime::residual` and offered
  again next substep, so an agent far from the origin keeps travelling instead of silently freezing
  (at 2e38 m one ULP is about 4e31 m, so a slow agent there would never move without this).
* **A substep's path is checked, not only its endpoint.** Every shortened candidate is accepted
  only when both ends and the midpoint of the swept segment lie in the mesh: the candidates all sit
  on one ray, so an endpoint-only test could cross an obstacle and land inside again. The check
  goes through `NavMesh`'s public point query, which keeps motion independent of the mesh's
  internals. It is a midpoint test, not the interval test routing uses
  (`MeshTopology::segment_is_contained`): an agent is a point, its swept volume is not tested, and
  an obstacle thinner than the midpoint test can notice may still be crossed inside one substep.
...
7. **Containment-safe integration rather than post-hoc repair.** A stride is capped by the
   distance to the target - carried-over remainder included - and, if the step would leave the mesh,
   halved repeatedly (bounded iterations), each candidate accepted only when both ends and the
   midpoint of the swept segment are inside, before the agent moves at all. Motion therefore stays continuous and inside
