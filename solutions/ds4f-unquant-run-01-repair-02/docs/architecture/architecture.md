...
- **Key contracts**: validation of every setup/mutation call with the exact
  ...
  is `Moving`. Pairwise avoidance is ORCA-style velocity-obstacle cones with a
  per-agent half-plane constraint per pair; every pair on a predicted collision
  course (within `kHorizon`) receives a deterministic escape split symmetrically
  between moving agents (full escape when the partner is stationary); pairs
  closer than the enlarged envelope `r_a + r_b + kEnvelopeMargin` are always in
  conflict and forced radially apart to the envelope, so passes keep at least
  the envelope (and the hard `r_a + r_b` bound) of separation. No traffic-side
  state: there is no `lane_side` pinning and no direction convention; the side
  of a pass is decided by the geometry of the encounter only.
...
- **Avoidance seam**: decision logic lives in one deterministic
  `advance(agents, mesh, dt, pair_esc)` substep function inside `simulation.cpp`,
  kept separate from state mutation (apply phase) and from input validation
  (public API); `pair_esc` is the `Impl`-owned sticky escape memory keyed by the
  ordered pair index, so decisions depend only on the snapshot plus the
  previously-issued escapes (determinism preserved).
...
| ORCA-style velocity-obstacle cones + half-plane constraints on a pre-update snapshot | Deterministic, local, no framework: for each close pair (i < j order) on a predicted collision course, the minimal change that clears the velocity obstacle is computed as the nearest point of the cone boundary (rays at ±α around the axis); each moving agent gets the half-plane `v·n ≥ low` on the opposite sides of the same escape, so satisfying both destroys the predicted collision. A sticky per-pair escape (released only after physical separation beyond the envelope) prevents snap-back oscillation; the radial envelope escape inside `r_cone` forces separation instead of allowing inward drift; the degenerate apex case (charging head-on) uses a deterministic lateral escape instead of a reversal, so exactly-equal symmetric encounters always resolve (SIM-010, SIM-012). No `lane_side`/traffic-side state. |
