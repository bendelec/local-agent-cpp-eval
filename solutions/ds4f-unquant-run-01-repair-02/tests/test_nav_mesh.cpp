...
TEST_CASE(nav_mesh_accepts_boundary_contacts) {
  ...
  CHECK(NavMesh::create(
            ...
            .has_value());
  // T-junction: the top triangle's base edge (0,0)-(2,0) is split into two
  // edges of the two bottom triangles; the vertex (1,0) sits on the open
  // edge of the top triangle, which would otherwise be silently treated as
  // boundary — the mesh validator rejects it
  CHECK_ERROR((NavMesh::create({tri({0, 0}, {2, 0}, {1, 1}),
                                tri({0, 0}, {1, -1}, {1, 0}),
                                tri({1, 0}, {1, -1}, {2, 0})})),
              ErrorCode::InvalidMesh);
