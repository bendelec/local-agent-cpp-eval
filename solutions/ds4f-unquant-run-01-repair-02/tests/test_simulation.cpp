...
// A huge but finite duration must neither overflow the substep conversion
// (float->size_t) nor loop unboundedly: the agent still ends finite,
// contained, and speed bounded.
void test_sim_step_large_finite_duration() {
  const NavMesh mesh = make_mesh(square_mesh());
  Simulation sim(mesh);
  const auto id =
      sim.add_agent(AgentConfig{.position = {0.5f, 0.5f}, .max_speed = 1.0f});
  ...
  CHECK(sim.step(1e9f).has_value());    // enormous but finite
  CHECK(sim.step(3.4e38f).has_value()); // near float max
  CHECK(sim.step(1e-30f).has_value());  // tiny positive
  CHECK(sim.step(0.0f).has_value());    // no-op

  const auto a = sim.agent(*id);
  CHECK(a.has_value());
  CHECK(std::isfinite(a->position.x) && std::isfinite(a->position.y));
  CHECK(mesh.contains(a->position));
  CHECK(std::isfinite(a->velocity.x) && std::isfinite(a->velocity.y));
  CHECK(length(a->velocity) <= 1.0f + 1e-4f);
  CHECK(a->status == AgentStatus::Reached);
}

// A faster follower overtaking a slower leader in the same lane must keep at
// least the sum of radii of separation, and both must reach their goals.
void test_sim_same_lane_overtake_separation() {
  const NavMesh mesh = make_mesh(big_square_mesh());
  Simulation sim(mesh);
  const auto slow = sim.add_agent(AgentConfig{
      .position = {1.5f, 2.0f}, .radius = 0.25f, .max_speed = 0.5f});
  const auto fast = sim.add_agent(AgentConfig{
      .position = {0.8f, 2.0f}, .radius = 0.25f, .max_speed = 1.5f});
  CHECK(slow.has_value() && fast.has_value());
  CHECK(sim.set_goal(*slow, {2.5f, 2.0f}).has_value());
  CHECK(sim.set_goal(*fast, {3.2f, 2.0f}).has_value());

  const float r_sum = 0.5f;
  float min_gap = std::numeric_limits<float>::infinity();
  for (int i = 0; i < 600; ++i) {
    CHECK(sim.step(0.05f).has_value());
    const auto sa = sim.agent(*slow);
    const auto sb = sim.agent(*fast);
    min_gap = std::min(min_gap, dist(sa->position, sb->position));
    CHECK(min_gap + 1e-3f >= r_sum);
    if (sa->status == AgentStatus::Reached &&
        sb->status == AgentStatus::Reached) {
      break;
    }
  }
  CHECK(sim.agent(*slow)->status == AgentStatus::Reached);
  CHECK(sim.agent(*fast)->status == AgentStatus::Reached);
}

// Two agents in close convoy around an L-shaped reflex corner: from the first
// step both must be Moving, separation must stay at the sum of radii, and
// both must reach their goals.
void test_sim_l_corner_convoy_progress() {
  Polygon l;
  l.vertices = {{0, 0}, {2, 0}, {2, 1}, {1, 1}, {1, 2}, {0, 2}};
  const auto tris = triangulate_simple_polygon(l);
  CHECK(tris.has_value());
  if (!tris) {
    return;
  }
  const auto mesh = NavMesh::create(*tris);
  CHECK(mesh.has_value());
  if (!mesh) {
    return;
  }
  Simulation sim(*mesh);
  const auto lead = sim.add_agent(AgentConfig{
      .position = {1.7f, 0.4f}, .radius = 0.15f, .max_speed = 1.0f});
  const auto folw = sim.add_agent(AgentConfig{
      .position = {1.2f, 0.4f}, .radius = 0.15f, .max_speed = 0.5f});
  CHECK(lead.has_value() && folw.has_value());
  CHECK(sim.set_goal(*lead, {0.3f, 1.55f}, 0.25f).has_value());
  CHECK(sim.set_goal(*folw, {0.5f, 1.0f}, 0.25f).has_value());

  CHECK(sim.agent(*lead)->status == AgentStatus::Moving);
  CHECK(sim.agent(*folw)->status == AgentStatus::Moving);

  const float r_sum = 0.3f;
  float min_gap = std::numeric_limits<float>::infinity();
  bool progressed = false;
  for (int i = 0; i < 800; ++i) {
    CHECK(sim.step(0.05f).has_value());
    const auto sa = sim.agent(*lead);
    const auto sb = sim.agent(*folw);
    min_gap = std::min(min_gap, dist(sa->position, sb->position));
    CHECK(min_gap + 1e-3f >= r_sum);
    if (i == 20) { // well into the corner transit
      progressed = dist(sa->position, (Vec2{0.3f, 1.55f})) <
                       dist(Vec2{1.7f, 0.4f}, (Vec2{0.3f, 1.55f})) &&
                   dist(sb->position, (Vec2{0.5f, 1.0f})) <
                       dist(Vec2{1.2f, 0.4f}, (Vec2{0.5f, 1.0f}));
    }
    if (sa->status == AgentStatus::Reached &&
        sb->status == AgentStatus::Reached) {
      break;
  ...
  CHECK(progressed);
  CHECK(sim.agent(*lead)->status == AgentStatus::Reached);
  CHECK(sim.agent(*folw)->status == AgentStatus::Reached);
}
...
int register_simulation_tests() {
  ...
  Registry::instance().add("sim_step_large_finite_duration",
                           &test_sim_step_large_finite_duration);
  Registry::instance().add("sim_same_lane_overtake_separation",
                           &test_sim_same_lane_overtake_separation);
  Registry::instance().add("sim_l_corner_convoy_progress",
                           &test_sim_l_corner_convoy_progress);
  return 0;
}
