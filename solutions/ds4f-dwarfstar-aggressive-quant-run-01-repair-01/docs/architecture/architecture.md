...
- **Dense-sampling contained-segment checks**: direct paths and corridor visibility
  edges are accepted only when every sample point is contained under MSH-006 (strictly
  inside or within `kEpsilon` of a triangle edge). This admits direct segments that
  cross a boundary edge within the tolerance zone, while rejecting segments that
  genuinely exit the mesh (e.g. across an L-notch) — exactly the SIM-002/SIM-003
...
- **Robust interior-overlap and self-intersection detection**: triangle interiors are
  rejected not only on strict point-in-triangle or proper edge crossings, but also on
  collinear edge overlap with the interiors on the same side (e.g. two identical
  triangles), while valid shared-edge adjacency stays accepted (MSH-004). Polygon
  validation likewise rejects self-touching outlines and overlapping non-adjacent
  collinear edges via point-on-segment and collinear-overlap checks (MSH-002).
