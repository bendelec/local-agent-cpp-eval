...
/// Shoelace signed double area of a closed ring; positive for counter-clockwise rings.
[[nodiscard]] float ring_double_area(const std::vector<Vec2> &vertices)
{
    const std::size_t count = vertices.size();
    float area = 0.0f;
    for (std::size_t i = 0; i < count; ++i)
    {
        area += cross(vertices[i], vertices[(i + 1) % count]);
    }
    return area;
}
...
[[nodiscard]] bool has_self_intersection(const std::vector<Vec2> &vertices)
{
    ...
    for (std::size_t i = 0; i < count; ++i)
    {
        ...
        for (std::size_t j = i + 1; j < count; ++j)
        {
            ...
            if (point_on_segment(b1, a1, a2, kEpsilon) || point_on_segment(b2, a1, a2, kEpsilon))
            {
                return true;
            }
        }
