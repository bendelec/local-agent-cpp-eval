...
struct AgentRuntime
{
    ...
    void adopt_route(std::vector<Vec2> new_route) noexcept
    {
        ...
        if (goal.has_value() && distance(*goal, position) <= arrival_radius)
        {
            route.clear();
            velocity = Vec2{};
        ...
        while (!new_route.empty() && distance(new_route.front(), position) <= kEpsilon)
        {
    ...
    void refresh_arrival() noexcept
    {
        ...
        if (distance(position, *goal) <= arrival_radius)
        {
            route.clear();
            velocity = Vec2{};
            status = AgentStatus::Reached;
