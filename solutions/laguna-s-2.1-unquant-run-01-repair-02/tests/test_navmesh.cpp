...
{
    // Two-vertex polygon.
    auto m1 = NavMesh::create({tri({0, 0}, {1, 0}, {1, 1}), Polygon{{Vec2{0, 0}, {1, 0}}}});
    EXPECT_FALSE(m1);
...
TEST(NavMeshCreate, NonManifoldEdgeRejected)
{
    ...
    auto m = NavMesh::create(
        {tri({0, 0}, {2, 0}, {2, 2}), tri({0, 0}, {2, 2}, {0, 2}), tri({0, 0}, {2, 2}, {4, 2})});
    EXPECT_FALSE(m);
    EXPECT_EQ(m.error().code, ErrorCode::InvalidMesh);
}
...
TEST(NavMeshCreate, OverlappingInteriorsRejected)
{
    ...
    auto m = NavMesh::create({tri({0, 0}, {4, 0}, {4, 4}), tri({1, 1}, {5, 1}, {1, 5})});
    EXPECT_FALSE(m);
    EXPECT_EQ(m.error().code, ErrorCode::InvalidMesh);
}

TEST(NavMeshCreate, VertexOnlyContactIsAccepted)
{
    // Two triangles that touch only at the single vertex (1,0): no shared edge,
    // no T-junction, no overlap. Previously rejected by the connected-fan
    // vertex check; now accepted (disjoint island / MSH-005).
    auto m = NavMesh::create({tri({0, 0}, {1, 0}, {1, 1}), tri({1, 0}, {2, 0}, {2, 1})});
    EXPECT_TRUE(m);
    EXPECT_EQ(m->cell_count(), 2u);
}

TEST(NavMeshCreate, VertexInEdgeInteriorIsTJunction)
{
    // tri1's vertex (2,0) lies in the interior of tri0's edge (0,0)-(4,0):
    // a genuine T-junction on a shared boundary.
    auto m = NavMesh::create({tri({0, 0}, {4, 0}, {4, 4}), tri({2, 0}, {6, 0}, {6, 2})});
    EXPECT_FALSE(m);
    EXPECT_EQ(m.error().code, ErrorCode::InvalidMesh);
}
