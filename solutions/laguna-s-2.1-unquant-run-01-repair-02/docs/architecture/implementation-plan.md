...
- Goal: `step` sub-stepped motion, waypoint overshoot guard, arrival, snapshot-based
  candidate-selection avoidance (c0–c6 goal-directed candidates + per-neighbour
  reciprocal separating/tangential-sidestep candidates, mesh-clamped via `add_cand`,
  selected by an 8-round Gauss-Seidel best-response with a tuple rank key)
  keeping agents in mesh and within max speed.
...
- **clang-tidy**: not configured in this environment (no `.clang-tidy` or
  `compile_commands.json` wired into `CMakeLists.txt`); changed `.cpp` sources are
  warning-clean under `-Wall -Wextra -Wpedantic` (see `docs/completion-report.md`).
