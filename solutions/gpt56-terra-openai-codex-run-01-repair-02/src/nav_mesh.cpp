...
constexpr double kSignedAreaEpsilon = kEpsilon * kEpsilon;
...
[[nodiscard]] DVec operator*(DVec value, double factor) noexcept {
...
// Twice signed triangle area, with subtraction and products performed in double.
...
[[nodiscard]] double distance_squared(Vec2 a, Vec2 b) noexcept {
    const DVec delta = difference(a, b);
...
[[nodiscard]] double point_segment_distance_squared(Vec2 point, Vec2 a, Vec2 b) noexcept {
    ...
    if (edge_length_squared == 0.0) {
        return dot(point_offset, point_offset);
    }
    const double projection = std::clamp(dot(point_offset, edge) / edge_length_squared, 0.0, 1.0);
    const DVec residual = point_offset + edge * -projection;
...
struct MeshAccess {
    ...
    static void append_circle_interval(std::vector<std::pair<double, double>> &intervals,
                                       Vec2 start, DVec delta, Vec2 centre) noexcept {
        const DVec offset = difference(start, centre);
        const double a = dot(delta, delta);
        const double b = 2.0 * dot(offset, delta);
        const double c = dot(offset, offset) - kEpsilon * kEpsilon;
        if (a == 0.0) {
            if (c <= 0.0) {
    ...
    static void append_cell_intervals(std::vector<std::pair<double, double>> &intervals,
                                      ...
                                      Vec2 end) noexcept {
        const DVec delta = difference(end, start);
        double low = 0.0;
        double high = 1.0;
        for (std::size_t edge = 0; edge < 3; ++edge) {
            const Vec2 a = cell.vertices[edge];
            const Vec2 b = cell.vertices[(edge + 1) % 3];
            const DVec direction = difference(b, a);
            clip_lower(cross(direction, delta), cross(direction, difference(start, a)), low, high);
        }
        if (low <= high) {
            intervals.emplace_back(low, high);
        }
        ...
        for (std::size_t edge = 0; edge < 3; ++edge) {
            ...
            const DVec start_offset = difference(start, a);
            clip_lower(dot(delta, direction), dot(start_offset, direction), rectangle_low,
                       rectangle_high);
            clip_lower(-dot(delta, direction), edge_length_squared - dot(start_offset, direction),
                       rectangle_low, rectangle_high);
            const double width = kEpsilon * std::sqrt(edge_length_squared);
            clip_lower(-cross(direction, delta), width - cross(direction, start_offset),
                       rectangle_low, rectangle_high);
            clip_lower(cross(direction, delta), width + cross(direction, start_offset),
                       rectangle_low, rectangle_high);
    ...
    [[nodiscard]] static bool segment_contained(const NavMesh::Impl &impl, Vec2 start, Vec2 end,
        ...
        if (start == end) {
            for (const auto &cell : impl.cells) {
                if ((component < 0 || cell.component == component) && cell_contains(cell, start)) {
                    return true;
                }
            }
            return false;
        ...
        double covered = 0.0;
        constexpr double kRoundingSlack = 1.0e-12;
        for (const auto &[low, high] : intervals) {
            if (low > covered + kRoundingSlack) {
                return false;
            }
            covered = std::max(covered, high);
            if (covered >= 1.0 - kRoundingSlack) {
                return true;
            }
        }
        return false;
...
Result<NavMesh> NavMesh::create(std::vector<Polygon> triangles) {
    ...
    for (const Polygon &triangle : triangles) {
        ...
        const std::array<Vec2, 3> vertices{triangle.vertices[0], triangle.vertices[1],
                                           triangle.vertices[2]};
        if (orient(vertices[0], vertices[1], vertices[2]) <= kTwiceAreaEpsilon) {
    ...
    for (std::size_t first = 0; first < impl->cells.size(); ++first) {
        for (std::size_t second = first + 1; second < impl->cells.size(); ++second) {
            ...
            for (const Vec2 vertex : a) {
                if (strictly_in_triangle(vertex, b)) {
