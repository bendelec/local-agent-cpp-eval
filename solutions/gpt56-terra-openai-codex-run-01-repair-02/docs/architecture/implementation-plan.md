...
documented before source edits; WP5 and WP6 record focused repair work.

## WP6 — Scale-safe geometry and route-continuing motion repair

This package is a **revision** for finite extreme-coordinate predicates and reflex-corner
motion endpoints. It preserves the public `Vec2` API while keeping geometric decisions private.

1. **F1: double-coordinate predicate seam** — **complete**  
   Goal: widen before all predicate differences/products, correct signed-area thresholds, and
   make finite-vector normalization robust. Files/interfaces: `src/geometry.cpp`,
   `src/nav_mesh.cpp`, `tests/vwmini_tests.cpp`. Verification: threshold, normalization, and
   finite extreme non-convex routing regressions.
2. **F2: route-continuing feasible motions** — **complete**  
   Goal: reject a mesh-limited stored endpoint unless routing can continue from it, refresh a
   valid route after a tangential portal bypass, and plan non-moving agents as stationary.
   Files/interfaces: `src/simulation.cpp`, `tests/vwmini_tests.cpp`. Verification: literal
   close-follower L-passage completion with per-step public invariants.
3. **F3: final quality evidence** — **complete**  
   Goal: apply checked formatting and rerun requested clean compiler, sanitizer, library-only,
   and CTest verification. Files/interfaces: project tree. Verification: recorded command
   outcomes in the completion report.
