...
inline bool isFiniteVec(Vec2 v) {
  return std::isfinite(v.x) && std::isfinite(v.y);
}
inline bool isFiniteFloat(float f) { return std::isfinite(f); }

static inline double cross_d(Vec2 a, Vec2 b) {
  return double(a.x) * b.y - double(a.y) * b.x;
}
static inline double dot_d(Vec2 a, Vec2 b) {
...
bool onSegment(Vec2 a, Vec2 b, Vec2 c) {
  return std::min(a.x, c.x) - 1e-9f <= b.x &&
         b.x <= std::max(a.x, c.x) + 1e-9f &&
...
struct EdgeQKey {
  QKey a, b;
  bool operator==(const EdgeQKey &o) const noexcept {
    return a == o.a && b == o.b;
  }
};
struct EdgeQHash {
...
triangulate_simple_polygon(const Polygon &polygon) {
  ...
  size_t n = verts.size();
  if (n < 3)
    return std::unexpected(Error{ErrorCode::InvalidMesh, "too few"});
  for (size_t i = 0; i < n; ++i)
    if (verts[i] == verts[(i + 1) % n])
      return std::unexpected(Error{ErrorCode::InvalidMesh, "duplicate"});
  for (size_t i = 0; i < n; ++i) {
    size_t i2 = (i + 1) % n;
    for (size_t j = i + 1; j < n; ++j) {
      ...
      if (segmentsIntersect(verts[i], verts[i2], verts[j], verts[j2]))
        return std::unexpected(
            Error{ErrorCode::InvalidMesh, "self-intersection"});
    }
  }
  float area = signedPolygonArea(verts);
  if (area <= 0.0f)
    return std::unexpected(Error{ErrorCode::InvalidMesh, "non-CCW"});
  ...
  while (idx.size() > 3) {
    ...
    for (size_t i = 0; i < m; ++i) {
      ...
      for (size_t j = 0; j < m; ++j) {
        ...
        if (pointInTri(p, a, b, c)) {
          if (pointOnSegmentEps(p, a, b) || pointOnSegmentEps(p, b, c) ||
              pointOnSegmentEps(p, c, a))
            continue;
          hasInside = true;
          break;
        }
    ...
    if (!earFound)
      return std::unexpected(
          Error{ErrorCode::InvalidMesh, "ear clipping failed"});
  }
  if (idx.size() == 3) {
...
Result<NavMesh> NavMesh::create(std::vector<Polygon> triangles) {
  if (triangles.empty())
    return std::unexpected(Error{ErrorCode::InvalidMesh, "empty"});
  ...
  for (auto &poly : triangles) {
    ...
    if (area2 <= 0)
      return std::unexpected(Error{ErrorCode::InvalidMesh, "clockwise"});
    if (std::fabs(area2) <= EPS * EPS)
      return std::unexpected(Error{ErrorCode::InvalidMesh, "degenerate"});
  ...
  for (auto &kv : edgeMap)
    if (kv.second.size() > 2)
      return std::unexpected(Error{ErrorCode::InvalidMesh, "non-manifold"});
  for (size_t i = 0; i < n; ++i) {
    for (size_t j = i + 1; j < n; ++j) {
      auto &ti = tris[i];
      auto &tj = tris[j];
      ...
      for (int ei = 0; ei < 3; ++ei) {
        ...
        for (int ej = 0; ej < 3; ++ej) {
          ...
          if (segmentsIntersect(a1, a2, b1, b2))
            return std::unexpected(
                Error{ErrorCode::InvalidMesh, "edge intersect"});
          if (pointOnSegmentEps(ti.v[ei], b1, b2) ||
              pointOnSegmentEps(ti.v[(ei + 1) % 3], b1, b2)) {
...
bool NavMesh::contains(Vec2 point) const noexcept {
  ...
  for (auto &t : m_impl->tris) {
    if (pointInTriangleStrict(point, t.v[0], t.v[1], t.v[2]))
      return true;
    for (int i = 0; i < 3; ++i)
      if (distPointSegment(point, t.v[i], t.v[(i + 1) % 3]) <= EPS)
        return true;
  }
  return false;
}
...
Result<Path> find_path(const NavMesh &mesh, Vec2 start, Vec2 goal) {
  ...
  auto segmentContained = [&](Vec2 a, Vec2 b) -> bool {
    if (!mesh.contains(a) || !mesh.contains(b))
      return false;
    ...
    for (auto &t : tris) {
      for (int e = 0; e < 3; ++e) {
        ...
        if (tpar >= -1e-9 && tpar <= 1 + 1e-9 && upar >= -1e-9 &&
            upar <= 1 + 1e-9) {
          ts.push_back(tpar);
        }
      }
  ...
  auto findContaining = [&](Vec2 p) -> int {
    for (size_t i = 0; i < tris.size(); ++i) {
      auto &t = tris[i];
      if (pointInTriangleStrict(p, t.v[0], t.v[1], t.v[2]))
        return (int)i;
      for (int e = 0; e < 3; ++e)
        if (distPointSegment(p, t.v[e], t.v[(e + 1) % 3]) <= EPS)
          return (int)i;
    }
    return -1;
  };
  ...
  for (int cur = gIdx; cur != -1; cur = prev[cur])
  ...
  for (size_t i = 0; i + 1 < triPath.size(); ++i) {
    ...
    for (int ea = 0; ea < 3 && !found; ++ea) {
      ...
      for (int eb = 0; eb < 3; ++eb) {
        ...
        if ((distPointSegment(a1, b1, b2) <= EPS &&
             distPointSegment(a2, b1, b2) <= EPS) ||
            (distPointSegment(b1, a1, a2) <= EPS &&
             distPointSegment(b2, a1, a2) <= EPS)) {
  ...
  for (auto &p : points)
    if (filtered.empty() || length(p - filtered.back()) > EPS)
...
Result<AgentId> Simulation::add_agent(const AgentConfig &cfg) {
  if (!isFiniteVec(cfg.position) || !isFiniteFloat(cfg.radius) ||
      !isFiniteFloat(cfg.max_speed))
    return std::unexpected(Error{ErrorCode::InvalidArgument, "non-finite"});
  ...
  if (cfg.goal) {
    ...
    if (!isFiniteVec(g))
      return std::unexpected(
          Error{ErrorCode::InvalidArgument, "goal non-finite"});
  ...
  ag.goal = cfg.goal;
  ag.arrival_radius_eff =
      (cfg.arrival_radius == -1.0f) ? cfg.radius : cfg.arrival_radius;
  if (ag.goal) {
    ...
    if (pr) {
      ...
      if (pr.error().code == ErrorCode::NoPath) {
...
Result<void> Simulation::remove_agent(AgentId id) {
  ...
  if (it == agents.end())
    return std::unexpected(Error{ErrorCode::NotFound, "not found"});
  agents.erase(it);
  return {};
}
...
Result<void> Simulation::set_goal(AgentId id, Vec2 goal, float arrival_radius) {
  if (!isFiniteVec(goal))
    return std::unexpected(
        Error{ErrorCode::InvalidArgument, "goal non-finite"});
  if (!validateArrivalRadius(arrival_radius))
    return std::unexpected(Error{ErrorCode::InvalidArgument, "arrival radius"});
  ...
  ag.goal = goal;
  ag.arrival_radius_eff =
      (arrival_radius == -1.0f) ? ag.radius : arrival_radius;
...
Result<void> Simulation::step(float seconds) {
  ...
  while (remaining > 1e-7f) {
    ...
    for (size_t i = 0; i < impl.agents.size(); ++i) {
      ...
      float d = length(to);
      if (d > EPS)
        desired[i] = normalized(to) * ag.max_speed;
      else
        desired[i] = {0, 0};
    }
    ...
    for (size_t i = 0; i < impl.agents.size(); ++i) {
      ...
      for (size_t j = 0; j < impl.agents.size(); ++j)
        if (j != i) {
          Vec2 otherPred = snapPos[j] + chosen[j] * dt;
          float dist = length(predPos - otherPred);
          if (dist < ag.radius + impl.agents[j].radius - 1e-4f) {
            collision = true;
            break;
          }
        }
      if (collision) {
        ...
        for (size_t j = 0; j < impl.agents.size(); ++j)
          if (j != i) {
            Vec2 otherPred = snapPos[j] + chosen[j] * dt;
            if (length(predPos - otherPred) <
                ag.radius + impl.agents[j].radius - 1e-4f) {
              stillColl = true;
              break;
            }
          }
