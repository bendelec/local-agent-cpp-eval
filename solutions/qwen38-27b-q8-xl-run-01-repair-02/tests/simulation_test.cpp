...
{
    // Scenario 2 (SIM-011): same lane, a faster agent (2.0 m/s) behind
    // overtakes a slower agent (1.0 m/s) ahead; both r = 0.25. The
    // trailing agent's goal (9,5) is 0.8 m from the leader's parking spot
    // (9.8,5) -- inside the degenerate zone (r <= dist < R = 0.9) of the
    // parked leader, so this also regression-covers the landing exemption
    // in the degenerate-zone slide: the trailing agent must still land a
    // goal that lies inside that zone (the discs stay non-overlapping:
    // 0.8 m > r_a + r_b = 0.5 m).
    const NavMesh mesh = squareMesh();
    Simulation sim(mesh);
    AgentConfig ca = configAt({1.f, 5.f});
...
{
    // The reported repro: 30 Hz public step duration. The subdivision
    // scheme (n = round(seconds / 0.1), min 1) uses ONE substep of 1/30 s
    // per call at this rate, so the failure was independent of substep
    // count. Pre-fix, the agent stalled at the reflex corner after ~4 s,
    // reporting Moving with zero motion.
...
TEST(S64_ReflexCorner, CloseFollowingPairBothReach)
{
    // A second, closely following agent (0.6 m behind, within the 1.5 m
    // avoidance lookahead + 0.4 m margin) must not stall the pair at the
    // reflex corner: the leader's turn puts the follower inside the leader's
    // local avoidance influence while both must still land the corner
    // waypoint and advance the route. The 1 m-wide arm cannot admit a
    // zero-clearance pass of two 0.25 m discs, so the follower's goal is
    // BEHIND the leader's (1.5 m apart, >= r_a + r_b): a feasible
    // non-passing case (SIM-011). 600 public calls of 1/60 s = 10 s of
    // simulation time.
    const NavMesh mesh = lMesh();
    Simulation sim(mesh);
    AgentConfig ca = configAt({2.5f, 0.5f});
    ca.max_speed = 1.4f;
    AgentConfig cb = configAt({3.1f, 0.5f});
    cb.max_speed = 1.4f;
    const AgentId a = *sim.add_agent(ca);
    const AgentId b = *sim.add_agent(cb);
    ASSERT_TRUE(sim.set_goal(a, {0.5f, 3.f}).has_value());
    ASSERT_TRUE(sim.set_goal(b, {0.5f, 1.5f}).has_value());
    ...
    EXPECT_TRUE(both_reached) << "agents stalled (reflex-corner close following)";
    const AgentState sa = *sim.agent(a);
    const AgentState sb = *sim.agent(b);
    EXPECT_NEAR(sa.position.x, 0.5f, 1e-3f);
    EXPECT_NEAR(sa.position.y, 3.f, 1e-3f);
    EXPECT_NEAR(sb.position.x, 0.5f, 1e-3f);
    EXPECT_NEAR(sb.position.y, 1.5f, 1e-3f);
}

// ---------------------------------------------------------------------------
// S65: regression for the overflow path in step(): the substep cap leaves a
// huge finite dt for huge finite durations; with avoidance cancelling the
// exact landing, the raw speed-bounded integration can overflow float and
// the containment clamp must not turn that into NaN (SIM-008/SIM-012).
// ---------------------------------------------------------------------------

TEST(S65_LargeDuration, HugeFiniteStepStaysFiniteAndContained)
{
    // Initially overlapping pair (SIM-010 robustness start) at non-default
    // max speeds: with seconds = float max, dt = seconds / kMaxSubsteps is
    // so large that |decided| * dt overflows float. Both agents would land
    // this substep (dist <= max_speed * dt), but the overlap push alters
    // their velocities, cancelling the landing, so the integration runs on
    // the speed-bounded avoided velocity. The call must succeed and every
    // state must remain finite and contained afterwards.
    const NavMesh mesh = squareMesh();
    Simulation sim(mesh);
    AgentConfig ca = configAt({5.f, 5.f});
    ca.max_speed = 1e7f;
    ...
    ASSERT_TRUE(sim.step(std::numeric_limits<float>::max()).has_value());
    ...
    for (const AgentId id : ids)
    {
        const AgentState s = *sim.agent(id);
        EXPECT_TRUE(std::isfinite(s.position.x) && std::isfinite(s.position.y))
            << "non-finite position for agent " << id.value;
        EXPECT_TRUE(std::isfinite(s.velocity.x) && std::isfinite(s.velocity.y))
            << "non-finite velocity for agent " << id.value;
        EXPECT_TRUE(mesh.contains(s.position))
            << "agent " << id.value << " outside mesh";
        EXPECT_LE(vwmini::length(s.velocity), s.max_speed + 1e-6f)
            << "velocity exceeds max_speed";
    }
    // The overlapping pair must be separated, not fused, by the push.
    const AgentState sa = *sim.agent(a);
    const AgentState sb = *sim.agent(b);
    EXPECT_GE(vwmini::length(sa.position - sb.position), 0.5f - 1e-3f);
}
