...
- **S6.2** [x] Avoidance: per-substep snapshot (SIM-010), RVO velocity-obstacle
  ...
  kAvoidWeight=1.0, kAvoidSeparationBoost=2.0 (revised post-review from
  0.7, see revision log). Verify: crossing pair,
...
- (S6.2, S6.3): Local disc avoidance implemented in `simulation.cpp` as
  ...
  max_speed * ((r-dist)/r + kAvoidSeparationBoost=2.0, revised post-review
  from 0.7, see revision log); coincident centres
...
- (post-S7.3 review fix 1) Reflex-corner stall: the route-index advance in
  `step` phase (e) fired on mere proximity (`dist <= max(1e-4, ms*dt)`)
  instead of on an exact landing. A substep whose remainder left the agent
  just short of a reflex-corner waypoint (or clamped into the boundary band
  past it) consumed the waypoint while the segment from that position to the
  next waypoint cut non-walkable space; the agent then steered into the wall,
  was clamped to ~zero motion, and reported Moving forever. Duration-
  dependent (30 Hz callers hit it). Fix: advance ONLY on exact waypoint
  landing (phase (e) now uses the `landing[k]` flag); the agent lands on the
  waypoint next substep and advances then. Regression: `S64_ReflexCorner`
  (L-shaped mesh, single agent, repro scenario at 30 Hz and 10 Hz, per-step
  containment/speed checks, Reached at the goal).
- (post-S7.3 review fix 2) Overlapping-disc non-separation: (a) the overlap
  response pushed self TOWARD the other agent (sign error: `v + u*magnitude`
  with `u` pointing self→other); flipped to `v - u*magnitude`. (b)
  `kAvoidSeparationBoost` 0.7 → 1.0 → 2.0: with boost <= 1 the push only
  cancels the worst-case mutual approach, leaving a stable equilibrium
  (permanent overlap at d = r*boost, or asymptotic contact at d = r for
  boost = 1); boost = 2.0 gives S >= ms_a + ms_b > 0 for every overlap
  depth. (c) The RVO cone projection was degenerate inside the clearance
  radius (`dist < R`: half-angle >= 90°, `w_boundary == 0`, "keep velocity"),
  so a head-on pair that separated back into the band steered straight into
  re-overlap; added direct centre-line repulsion (`2 * max_speed`, strictly
  positive separation rate) for that band. The old
  `OverlappingStartIsSeparated` test had hidden (a) by disabling the
  separation invariant; it now asserts strict separation growth while
  overlapping, recovery to a clear distance, maintained separation, and
  both agents reaching (plus a head-on overlapping variant). All 74 tests
  green and warning-clean on g++ and clang++; ASan/UBSan clean.
- (second review round) Three further fixes, all in `simulation.cpp` /
  `simulation_test.cpp`, no public header changes; final state 76 tests:
  (1) **Large finite `step()` durations** overflowed to NaN: (a)
  `n = llround(seconds/0.1)` is UB beyond ~9.2e17 s — the cap comparison
  now runs BEFORE `llround` (cap for `scaled >= kMaxSubsteps`, else
  `llround`), so large finite durations are accepted (clamped), never
  rejected; (b) the capped huge finite `dt` makes the displacement
  finite-but-huge or infinite, and the old [0,1]-parameter bisection
  clamp degenerated to `from + inf * 0 = NaN` (or needed ~120 bisections
  for a huge finite delta) — the clamp now works in arc length: exponential
  expansion to a definitely-outside bound (capped at `|delta|` when finite,
  preserving the old furthest-contained-point semantics), then bisection;
  `t = 0` contained ⇒ result always finite and contained; normal substeps
  unchanged (bit-identical). Regression: `S65_LargeDuration` (overlapping
  start, `max_speed = 1e7`, single `step(FLT_MAX)`, finite/contained/
  separated after). (2) **Close-following pair stalled at the reflex
  corner** (SIM-011): in the degenerate zone (`r <= dist < R`) the cone's
  ill-conditioned projection (direction dominated by a near-zero perp
  component whose sign flips with sub-mm position changes) plus the
  first-round centre-line repulsion formed a stable limit cycle — the
  follower flickered between the arm walls, the leader was deflected into
  the opposite wall, the containment clamp pinned both, and the corner
  waypoint was never landed. Fix: degenerate-zone **slide** replacing both
  the projection and the repulsion — approaching component dropped,
  lateral kept, outward drift `kAvoidSlideDrift = 0.25` of `max_speed`;
  receding agents left alone (the approaching partner's slide already grows
  the centre-line distance, which is strictly increasing in the zone:
  >= 2·drift·`ms` head-on / >= drift·`ms` one-sided ⇒ no re-overlap, bounded
  exit time, the leader clears the corner). A **landing agent** (within
  `max_speed*dt` of its own target) is exempt and glides to the stop
  (overlap push still guards contact) — required so an agent whose goal
  lies inside the zone of a parked agent can land it (same-lane overtaking
  regressed without the exemption: trailing goal 0.8 m from the leader's
  parking spot). Regression: `S64_ReflexCorner.CloseFollowingPairBothReach`
  (L-mesh, follower 0.6 m behind, goals 1.5 m apart >= r_a + r_b — the
  1 m arm cannot admit a zero-clearance pass, so the feasible non-passing
  case; per-step invariants; both Reached). (3) **Integrity**: the
  reconstructed `S62` overtaking and angled-crossing tests confirmed
  present and passing with separation assertions intact (the overtaking
  test directly exercises the landing exemption); the wrong "3 substeps
  per 30 Hz call" comments corrected in tests and docs (the scheme uses
  ONE 1/30 s substep per call: n = round((1/30)/0.1) = 1). All 76 tests
  green and warning-clean on g++ and clang++; ASan/UBSan clean.
