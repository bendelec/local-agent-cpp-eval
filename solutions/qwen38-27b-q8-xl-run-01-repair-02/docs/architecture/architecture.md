...
| `simulation` | Public `Simulation` PIMPL (SIM-005…013): agent storage and id lifecycle, goal state machine, deterministic substep stepping (snapshot → steer → avoid → integrate → clamp → transitions). Local disc avoidance (SIM-010…012) is implemented here: per-substep snapshot of all agents (positions + pre-avoidance steer velocities), waypoint steering clamped by max speed with exact waypoint landing (no overshoot), RVO velocity-obstacle projection (double cone in relative-velocity space; deterministic anti-symmetric fallback for coincident centres), degenerate-zone slide with landing exemption for `r <= dist < R` (see below), single integrated move with post-move containment clamp (arc-length expansion + bisection). Route indices advance ONLY on exact waypoint landing (never on mere proximity), so a consumed waypoint is always one the agent stands on, from where the path-validated next segment is walkable. Constants: `kAvoidLookahead = 1.5` m, `kAvoidMargin = 0.4` m, `kAvoidWeight = 1.0`, `kAvoidSeparationBoost = 2.0`, `kAvoidSlideDrift = 0.25`. |
...
5. **Avoidance = RVO velocity-obstacle projection, single-substep** (SIM-010,
   ...
   containment clamp (arc-length expansion to an outside bound, then
   bisection on that interval) is a safety net (SIM-012). The substep
   count `n = clamp(llround(seconds / 0.1))` guards the cap comparison
   BEFORE `llround`, so no UB for any finite `seconds` (a huge finite
   duration is clamped to the 1,000,000-substep cap and still accepted,
   per SIM-008); with the resulting huge finite `dt` the displacement can
   overflow float, and the arc-length clamp keeps the result finite and
   contained instead of producing NaN. Two degenerate regimes get direct
   handling (both found as review-time
   ...
   - **Inside the clearance radius, not overlapping** (`r_a + r_b <=
     ...
     degenerate (half-angle >= 90°, `w_boundary == 0`). Instead of the
     projection (whose direction in this zone is dominated by a near-zero
     perpendicular component whose sign flips with sub-millimetre position
     changes — a close-following pair at a reflex corner flickered between
     it and the first-round centre-line repulsion in a stable limit cycle
     and neither agent ever landed the corner waypoint), the pair **slides**:
     the approaching component of the velocity is dropped, the lateral
     component is kept, and the approaching agent gains an outward drift of
     `kAvoidSlideDrift * max_speed` (a receding agent is left alone — the
     approaching partner's slide already makes the centre-line distance
     grow). While both agents of a conflicting pair are in the zone the
     centre-line distance is strictly increasing (>= 2·drift·`max_speed`
     head-on, >= drift·`max_speed` one-sided), so the pair can never
     re-enter overlap from the zone and leaves it in bounded time; the
     leader advances cleanly and clears the corner. A **landing agent**
     (within `max_speed*dt` of its own target waypoint) is exempt and
     glides to the stop uncorrected — without the exemption an agent whose
     goal lies inside the zone of a parked agent (e.g. same-lane overtaking
     with the trailing goal 0.8 m from the leader's parking spot) could
     never land it; the overlap push below still guards physical contact
     at the landing point.
   Substep duration is fixed (10 Hz internal clock, capped at 1,000,000
   substeps so total steps bound runtime deterministically; large finite
   `seconds` are clamped to the cap, never rejected).
