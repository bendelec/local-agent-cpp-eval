...
Result<void> Simulation::step(float seconds)
{
    ...
    // Bound the substep count so huge but finite durations (e.g. FLT_MAX) cannot
    // push seconds/target_substep past FLT_MAX and turn the `ceil` -> `size_t`
    // cast into undefined behaviour. Values larger than this are clamped; for any
    // in-mesh goal reachable at max_speed, the clamped time is already plenty.
    constexpr float max_effective_seconds = 1'000.0f;
    const float effective = seconds <= max_effective_seconds ? seconds : max_effective_seconds;
    const std::size_t n_sub =
        std::max<std::size_t>(1, static_cast<std::size_t>(std::ceil(effective / target_substep)));
    const float sub_dt = effective / static_cast<float>(n_sub);
    ...
    for (std::size_t s = 0; s < n_sub; ++s) {
        ...
        for (auto* a : live) {
            ...
            snap.push_back({a->position, a->velocity, dest, a->radius, a->max_speed,
                            a->status == AgentStatus::Moving});
        }
            ...
            for (std::size_t i = 0; i < snap.size(); ++i) {
                for (std::size_t j = i + 1; j < snap.size(); ++j) {
                    ...
                    if (dist > 0.0f) {
                        ...
                        // Coincident centres (deterministic tie-break): push the
                        // pair apart along a fixed axis so they can never fuse into
