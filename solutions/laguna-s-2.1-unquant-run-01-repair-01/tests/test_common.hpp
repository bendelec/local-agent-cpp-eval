...
// L-shaped walkable corridor: bottom strip [0,3]x[0,1] and right stem
// [2,3]x[0,3]. Contains a genuine reflex corner at (2,1) so a straight
// start->goal segment through the notch leaves the mesh.
inline NavMesh make_L_corridor()
{
    const Polygon L{
        {{Vec2{0.0f, 0.0f}, {3.0f, 0.0f}, {3.0f, 3.0f}, {2.0f, 3.0f}, {2.0f, 1.0f}, {0.0f, 1.0f}}}};
    auto tris = triangulate_simple_polygon(L);
    assert(tris);
    auto r = NavMesh::create(*std::move(tris));
    assert(r);
    return *std::move(r);
}
