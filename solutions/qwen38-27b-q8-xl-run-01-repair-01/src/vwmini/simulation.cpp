                                      ...
                                      const std::vector<Vec2>& steer)
{
    ...
    for (std::size_t j_k = 0; j_k < live_slots.size(); ++j_k)
    {
        ...
        else
        {
            ...
            if (dist < R)
            {
                // Inside the clearance radius: the velocity obstacle covers
                // the ENTIRE feasible set -- the pair is already closer than
                // the clearance, so no relative velocity can keep the
                // predicted closest approach >= R. There is nothing to
                // project onto, so repel directly away from the other agent.
                // 2 * max_speed per agent guarantees a strictly positive
                // relative separation rate for any speed combination (worst
                // case both steer straight at each other: S >= 2*ms_a +
                // 2*ms_b - (ms_a + ms_b) = ms_a + ms_b > 0), so the pair
                // leaves the band with guaranteed progress and the
                // well-defined cone (or a lateral pass) takes over above R.
                // Without this, the degenerate cone below (half-angle >=
                // 90 deg, w_boundary == 0) would "keep" the velocity and a
                // head-on pair would steer straight back into overlap.
                v_new = v - u * (2.0f * max_speed);
            }
            else
            {
