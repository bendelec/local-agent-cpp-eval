...
- MSH-001: extreme finite inputs saturate instead of overflowing: `length` of a vector
  whose true magnitude is not representable as `float` returns `FLT_MAX` (finite), and
  `normalized` of any finite non-zero vector returns a unit vector even where a
  float-computed length would overflow. Non-finite inputs are outside the finite-input
  guarantee; `length` nonetheless stays finite, saturating at `FLT_MAX`.
