...
| NFR-010 | Deliverable | Implementation work is planned in small verifiable slices. | Before editing library source, create `docs/architecture/implementation-plan.md`. Divide the work into ordered work packages and small slices. For each slice state its goal, dependencies, expected files/interfaces, verification, and completion status. Follow the plan; revise it explicitly when evidence changes a decision rather than silently abandoning it. The plan's decomposition is the implementer's design choice. |
