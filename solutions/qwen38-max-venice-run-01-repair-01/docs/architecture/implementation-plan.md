...
- [x] **S4.3 `step` validation + single-agent movement.** Goal: negative/
  ...
  (the ≤ 1/60 s bound holds only up to the 4096-substep cap — durations beyond
  ~68.3 s receive coarser substeps; see WP5 review-fix-2);
...
- [x] **S4.4 Avoidance.** Goal: snapshot read before updates (SIM-010);
  ...
  (now charged only for `Moving` neighbors — see WP5 review-fix-2).
...
- [x] **S5.3 Final verification & commits.** Full clean build, ctest green,
  ...
  - **Outcome (S5.3):** A fresh out-of-tree configure + Release build is
    ...
    is complete.**
- [x] **S5.4 Second review-fix pass.** Goal: reproduce, fix, and regression-test
  four findings from a deep code review (extreme-finite geometry saturation;
  ULP overshoot of the per-substep budget; a follower permanently stranded
  behind a Reached leader in a barely-passable corridor; wrapping id
  allocator at exhaustion) plus a contradictory substep doc comment, and gate
  the test tree behind `BUILD_TESTING`. Deps: S5.3. Verify: each behavioral
  regression test demonstrated failing against the pre-fix code (the
  id-allocator regressions are white-box tests of the new checked component —
  exhaustion is unreachable through the frozen public API in a test
  lifetime); full matrix (GCC/Clang
  × Debug/Release, ASan+UBSan, `BUILD_TESTING=OFF`, clang-format).
  - **Outcome (S5.4):** All findings reproduced and fixed (details and
    evidence in the WP5 review-fix-2 revision below); 107/107 tests green in
    all five build configurations; library-only build warning-free without
    GTest; public headers remain byte-identical to f998008.

## Revisions

Deviations from the plan above, recorded newest-first with their evidence:

- **WP5 revision (review-fix 2, S5.4):** A deep-review pass surfaced four
  defects plus one doc contradiction; each was fixed together with a
  regression test, the behavioral ones demonstrated to fail against the
  pre-fix code. Evidence:
  107/107 tests green under GCC 16.2 and Clang 22 × Debug/Release plus
  ASan+UBSan; `BUILD_TESTING=OFF` configures and builds without GTest;
  repo-wide clang-format clean; public headers unchanged (f998008).
  - **MAJOR — geometry overflow on extreme finite inputs.**
    `length({FLT_MAX, FLT_MAX})` returned +inf and `normalized` propagated it
    (inf/inf → NaN components for a finite input), violating MSH-001's
    finite-input semantics. Both now compute in double (`std::hypot`):
    `length` saturates at `FLT_MAX`; `normalized` divides in double so every
    finite non-zero input yields a true unit vector. Regressions:
    `Geometry.LengthSaturatesWhenTrueMagnitudeExceedsFloatRange`,
    `Geometry.NormalizedHandlesVectorsWhoseFloatLengthOverflows`.
  - **MAJOR — ULP overshoot of the SIM-008 budget.** The walk_route partial
    move and the detour trial stored `from + dir·budget` in float, which can
    round one ULP *above* the `max_speed·dt` cap. A new `clamp_step()` helper
    pulls the landing toward the start with `std::nextafter` until the stored
    displacement fits the cap (no-op substep when no representable step
    fits). Regression:
    `SimulationStep.TinyStepAtLargeCoordinatesNeverOvershootsBudget`.
  - **MAJOR — follower permanently stranded behind a Reached leader.** The
    replanning margin `2·(s_i+s_j)·dt` was charged for *every* neighbor in
    the horizon. For a parked (Reached) neighbor — which executes no motion
    and whose snapshot is therefore exact — this demanded more clearance than
    a corridor barely wider than the radius sum can offer: reproduced with a
    0.64 m passing gap versus ~0.647 m demanded at the dt ≈ 1/120 substeps
    that `step(1/60)` splits into (`float(1/60) > double(1/60)` → two
    substeps), freezing the follower at the wall at zero velocity, Moving
    forever. The margin is now charged only for `Moving` neighbors
    (`FrameAgent::mobile`); the radius-sum floor is never relaxed.
    Regression:
    `SimulationAvoidance.FollowerPassesReachedLeaderInTightCorridor`.
  - **MAJOR — wrapping id allocator.** `acquire_id` wrapped to 1 at 32-bit
    exhaustion, silently reusing live ids against SIM-005's never-reuse
    guarantee. Replaced by a checked `internal::AgentIdAllocator`
    (`src/vwmini/internal/agent_id.hpp`): `acquire()` returns
    `std::optional<uint32_t>`, exhaustion is sticky, and `add_agent` fails
    transactionally with `InvalidArgument` before any mutation. White-box
    regressions: `tests/agent_id_test.cpp` (3 tests; `tests/` gains the
    `src/` include directory for internal components).
  - **MINOR — contradictory substep documentation.** The module comment
    claimed substeps of “at most 1/60 s” while durations beyond ~68.3 s
    (4096-substep cap) receive coarser ones. Comment, architecture.md, and
    the SIM-008 edge case now state the actual policy.
  - **BUILD — `BUILD_TESTING` gate.** The top-level CMakeLists defines
    `option(BUILD_TESTING … ON)` gating `enable_testing()` and
    `add_subdirectory(tests)`, so library-only builds never require GTest
    (NFR-001).
