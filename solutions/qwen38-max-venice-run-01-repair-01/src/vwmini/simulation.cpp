...
// removed id keeps returning `NotFound` forever (SIM-013); once the 32-bit id
// space is exhausted, `add_agent` reports InvalidArgument instead of wrapping
// to possibly-live ids. Removed slots are recycled through a free list.
//
// Stepping (SIM-008/010): `step` splits the duration into uniform substeps of
// at most 1/60 s, up to 4096 of them per call; longer durations (~68 s and
// beyond) receive fewer, coarser substeps instead, so huge finite durations
// stay responsive while route walking keeps motion exact and bounded
// regardless of dt. Each substep runs in two phases: first every live agent is snapshotted and
...
struct FrameAgent {
    ...
    bool detoured{false};
    /// True only while the agent is Moving: non-Moving agents execute no
    /// motion this substep, so their snapshot cannot go stale during scoring.
    bool mobile{false};
};
...
/// Rounds a planned landing down to the motion budget. The motion math runs in
/// double, but the stored position is float: at large coordinates the float
/// quantum can exceed a whole substep budget (ULP ~1e-3 at |x| ~ 1e4, while a
/// 1.4 m/s agent travels ~8e-4 m in a 0.6 ms substep), so the naive
/// double->float round trip can overshoot the budget by one ULP and violate
/// SIM-008's max_speed * dt cap. Pulling the landing toward `from` with
/// nextafter until the stored displacement fits keeps the cap exact; when no
/// representable step fits the budget, the result is `from` itself and the
/// substep executes no motion.
[[nodiscard]] Vec2 clamp_step(Vec2 from, Vec2 candidate, double budget)
{
    Vec2 p = candidate;
    while (distance(from, p) > budget) {
        // std::nextafter(x, y) already yields x when x == y, so a component at
        // `from` stops moving and `next == p` once both have arrived.
        const Vec2 next{std::nextafter(p.x, from.x), std::nextafter(p.y, from.y)};
        if (next == p) { // Componentwise at `from`; cannot pull closer.
            return from;
        }
        p = next;
    }
...
struct Simulation::Impl {
    ...
    [[nodiscard]] Result<AgentId> add_agent(const AgentConfig& config)
    {
        ...
        // Checked before any mutation: an exhausted id space fails the add
        // with InvalidArgument instead of wrapping to possibly-live ids
        // (SIM-005).
        const std::optional<std::uint32_t> new_id = id_allocator.acquire();
        if (!new_id.has_value()) {
            return make_error(ErrorCode::InvalidArgument, "agent id space exhausted");
        }
    ...
    void run_substep(double dt)
    {
        ...
        for (std::size_t s = 0; s < slots.size(); ++s) {
            ...
            frame.push_back(FrameAgent{.slot = s,
                                       .position = rec.position,
                                       .velocity = rec.velocity,
                                       .radius = rec.radius,
                                       .max_speed = rec.max_speed,
                                       .mobile = rec.status == AgentStatus::Moving});
        }
    ...
    /// replanning against Moving neighbors (a parked neighbor's snapshot is
    /// exact, so no margin is charged for it). The winner is the feasible candidate with the most
    /// progress toward the goal, or, when none is feasible, the one maximizing predicted separation
    /// (so overlapping agents actively separate).
                                            ...
                                            double dt) const
    {
        ...
        for (const std::size_t j : neighbors) {
            ...
            // Margin absorbing simultaneous replanning (SIM-010): each Moving
            // agent's executed substep displacement can deviate from its
            // snapshot prediction by up to 2 * max_speed * dt, so the executed
            // pairwise distance never dips below the sum of radii. A
            // non-Moving neighbor (Idle/Reached/NoPath) executes no motion at
            // all — apply_movement holds it exactly at its snapshot position —
            // so its prediction carries no error and no margin is charged.
            // Charging a parked agent's max_speed anyway made the required
            // clearance exceed what a corridor barely wider than the two discs
            // geometrically permits, stranding a follower behind an arrived
            // leader forever. The radius-sum floor itself is never relaxed.
            const double margin =
                other.mobile ? 2.0 * (double(rec.max_speed) + double(other.max_speed)) * dt : 0.0;
            const double required = double(rec.radius) + double(other.radius) + margin;
            worst_deficit =
    ...
    void apply_movement(AgentRecord& rec, Vec2 plan, bool detoured, double dt)
    {
        ...
        if (detoured) {
            // clamp_step first: the raw float landing can overshoot the
            // double-precision budget by one ULP at large coordinates (SIM-008).
            const Vec2 trial = clamp_step(old, old + plan * float(dt), budget);
    ...
    void walk_route(AgentRecord& rec, double budget) const
    {
        ...
        while (budget > 0.0 && rec.next_waypoint <= last) {
            ...
            if (d > budget) {
                const Vec2 from = rec.position;
                const D2 dir = normalized2(to_d(target) - to_d(from));
                // clamp_step keeps the float landing within the remaining budget:
                // the double->float round trip can overshoot by one ULP at large
                // coordinates (SIM-008).
                rec.position = clamp_step(from, to_vec(to_d(from) + dir * budget), budget);
                return;
            }
            rec.position = target;
            budget -= d;
    ...
    /// Monotonic id source; reports exhaustion instead of reusing live ids.
    internal::AgentIdAllocator id_allocator;
