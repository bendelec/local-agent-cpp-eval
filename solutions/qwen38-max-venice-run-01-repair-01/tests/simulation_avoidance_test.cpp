...
/// A corridor wide enough to pass a parked agent only just: the leader stops
/// at x = 0.31, the east wall is x = 0.95, so the best passing separation is
/// 0.64 — above the no-overlap floor (two radii = 0.60) and therefore legal.
[[nodiscard]] NavMesh tight_corridor_mesh()
{
    return make_mesh(
        outline_triangles({V(0.0f, 0.0f), V(0.95f, 0.0f), V(0.95f, 3.0f), V(0.0f, 3.0f)}));
}

TEST(SimulationAvoidance, FollowerPassesReachedLeaderInTightCorridor)
{
    // Regression: the replanning margin used to charge even a mere Reached
    // neighbor its full max_speed (2 * 2.8 * dt ~= 0.047 at the dt ~= 1/120
    // substeps that step(1/60) splits into), demanding ~0.647 > 0.64. No
    // candidate was ever feasible, the fallback kept the follower pinned to
    // the wall at zero velocity, Moving forever. With zero margin for
    // immobile neighbors the pass is feasible and both agents reach goals.
    const NavMesh mesh = tight_corridor_mesh();
    Simulation sim(mesh);
    // The leader runs straight up the west side and parks within its arrival
    // radius of (0.31, 2.5); the follower starts south-east of it with a goal
    // beyond, on the passing (east) side.
    const AgentId leader = add_or_fail(sim, cfg(V(0.31f, 1.20f), 1.4f, V(0.31f, 2.50f)));
    const AgentId follower = add_or_fail(sim, cfg(V(0.60f, 0.30f), 1.4f, V(0.75f, 2.70f)));

    const RunSummary run = run_pair(sim, leader, follower, mesh);
    EXPECT_TRUE(run.ok) << run.failure << " at step " << run.failure_step;
    // The heart of the regression: the follower must not stay stranded behind
    // the parked leader; both goals are reachable well within the budget.
    EXPECT_TRUE(run.both_reached) << "steps taken: " << run.steps_taken;
    // Passing must not relax the no-overlap floor: separation stays at or
    // above the radius sum (minus the documented tolerance) throughout.
    EXPECT_GE(run.min_separation, kEnforce);
    const AgentState parked = state_or_fail(sim, leader);
    EXPECT_EQ(parked.status, AgentStatus::Reached);
    EXPECT_LT(length(parked.velocity), 1e-6f);
}
