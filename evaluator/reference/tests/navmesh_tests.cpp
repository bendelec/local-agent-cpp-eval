// Navmesh and path track (N): triangle-mesh validation (MSH-004), containment
// (MSH-006), and find_path direct/disconnected/bent behavior (SIM-001..SIM-004).
...
TEST(FindPath, BendsAtReflexCornerRatherThanPortalMidpoint)
{
    const NavMesh mesh = make_l_mesh();
    const Vec2 start{1.75f, 0.75f};
    const Vec2 goal{0.25f, 1.75f};
    const auto result = find_path(mesh, start, goal);
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->points.size(), 3u);
    EXPECT_EQ(result->points[0], start);
    EXPECT_EQ(result->points[1], (Vec2{1.0f, 1.0f}));
    EXPECT_EQ(result->points[2], goal);
}
