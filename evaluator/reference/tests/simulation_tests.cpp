...
TEST(Simulation_Step, InitiallyOverlappingAgentsAttemptToSeparate)
{
    Simulation sim(make_square_mesh());
    AgentConfig config;
    config.position = Vec2{5.0f, 5.0f};
    config.radius = 0.25f;
    const auto first = sim.add_agent(config).value();
    const auto second = sim.add_agent(config).value();

    ASSERT_TRUE(sim.step(1.0f / 30.0f).has_value());
    const auto first_state = sim.agent(first);
    const auto second_state = sim.agent(second);
    ASSERT_TRUE(first_state.has_value());
    ASSERT_TRUE(second_state.has_value());
    EXPECT_GT(length(first_state->position - second_state->position), 0.0f);
    EXPECT_TRUE(is_finite(first_state->position));
    EXPECT_TRUE(is_finite(second_state->position));
}
...
TEST(Simulation_Query, MovedFromSimulationIsSafeToQuery)
{
    Simulation original(make_square_mesh());
    Simulation moved(std::move(original));
    EXPECT_EQ(original.agent_count(), 0u);
    EXPECT_FALSE(original.agent(AgentId{1u}).has_value());
    expect_error(original.step(0.1f), ErrorCode::InvalidArgument);
    expect_error(original.add_agent(AgentConfig{}), ErrorCode::InvalidArgument);
    EXPECT_EQ(moved.agent_count(), 0u);
}
