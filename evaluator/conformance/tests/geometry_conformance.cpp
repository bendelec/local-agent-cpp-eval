...
//
// This track calls only geometry.hpp: no NavMesh, Simulation, or crowd fixture.
...
TEST(Geometry_Vec2, DotAndCross)
{
    ...
    EXPECT_EQ(dot(Vec2{1.0f, 0.0f}, Vec2{0.0f, 1.0f}), 0.0f); // orthogonality
    EXPECT_EQ(cross(Vec2{1.0f, 0.0f}, Vec2{0.0f, 1.0f}), 1.0f); // CCW sign
}
...
{
    // A square with an extra collinear point on the bottom edge is valid. An
    // implementation may retain that input vertex (three triangles) or remove it
    // during preprocessing (two triangles); both satisfy the public contract.
